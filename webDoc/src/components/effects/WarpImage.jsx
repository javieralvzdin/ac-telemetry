...
vec4 sampleImage(vec2 uv) {
  ...
  return texture(uImageTexture, uv);
}
...
void main() {
  ...
  vec4 base = sampleImage(displaced);
  float r = sampleImage(displaced + split).r;
  float g = base.g;
  float b = sampleImage(displaced - split).b;
  float a = max(max(sampleImage(displaced + split).a, base.a), sampleImage(displaced - split).a);

  vec3 color = vec3(r, g, b) + lens * base.a * 0.06;
...
const WarpImage = ({
  ...
  const propsRef = useRef({ warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple });
  const contextRef = useRef(null);

  useEffect(() => {
    propsRef.current = { warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple };
    if (contextRef.current) syncUniforms(contextRef.current.program, propsRef.current);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WarpImage: WebGL could not be initialized.', error);
      return undefined;
    }
  ...
  return (
    <div ref={containerRef} className={`warp-image ${className}`.trim()} style={style} role="img" aria-label={alt} />
  );
};
